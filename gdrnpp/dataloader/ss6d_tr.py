...
for lv, b_dir in B_values.items():
    for rgb_mode in rgb_modes:
        key = f"ss6d_{lv.lower()}{rgb_mode.lower()}_tr"
        SPLITS_SS6D_TR[key] = dict(
            name=key,
            rgb_mode=rgb_mode,
            ...
            xyz_root=osp.join(DATASETS_ROOT, f"BOP_DATASETS/SenseShift6D/train/{b_dir}/xyz_crop"),
...
for obj in SS6D_OBJECTS:
    for lv, b_dir in B_values.items():
        for rgb_mode in rgb_modes:
            name = "ss6d_{}_{}{}_tr".format(obj, lv.lower(), rgb_mode.lower())
            if name not in SPLITS_SS6D_TR:
                SPLITS_SS6D_TR[name] = dict(
                    name=name,
                    rgb_mode=rgb_mode,
                    depth_mode=DEPTH_MODE,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_SS6D_TR:
        used_cfg = SPLITS_SS6D_TR[name]
    else:
...
def get_available_datasets():
    return list(SPLITS_SS6D_TR.keys())
