...
class SS6D_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/SenseShift6D"))
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.ss6d.id2obj.items() if obj_name in self.objs]
        ...
        self.scenes = [f"{i:06d}" for i in range(NUM_SCENES)]
    ...
    def __call__(self):
        ...
        for scene in tqdm(self.scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                rgb_path = osp.join(scene_root, "rgb", self.rgb_mode, f"{int_im_id:06d}.png")
                assert osp.exists(rgb_path), rgb_path

                depth_path = osp.join(scene_root, "depth", self.depth_mode, f"{int_im_id:06d}.png")
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    # print("mask_rle shape:", mask_rle.astype("float32").shape)
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.ss6d.obj2id[obj_name]:06d}.ply",
                ),
...
def get_SS6D_metadata(obj_names, ref_key):
...
rgb_modes = ["AE", "E9G16", "E9G48", "E9G80", "E9G112",
...
for lv, b_dir in B_values.items():
    for rgb_mode in rgb_modes:
        for depth_mode in depth_modes:
            key = f"ss6d_{lv.lower()}{rgb_mode.lower()}_d{depth_mode}_te"
            SPLITS_SS6D_TE[key] = dict(
                name=key,
                rgb_mode=rgb_mode,
                depth_mode=depth_mode,
                objs=SS6D_OBJECTS,
                dataset_root=osp.join(DATASETS_ROOT, f"BOP_DATASETS/SenseShift6D/test/{b_dir}"),
                models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/SenseShift6D/models"),
                xyz_root=osp.join(DATASETS_ROOT, f"BOP_DATASETS/SenseShift6D/test/{b_dir}/xyz_crop"),
...
for obj in SS6D_OBJECTS:
    for lv, b_dir in B_values.items():
        for rgb_mode in rgb_modes:
            for depth_mode in depth_modes:
                name = "ss6d_{}_{}{}_d{}_te".format(obj, lv.lower(), rgb_mode.lower(), depth_mode)
                if name not in SPLITS_SS6D_TE:
                    SPLITS_SS6D_TE[name] = dict(
                        name=name,
                        rgb_mode=rgb_mode,
                        depth_mode=depth_mode,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_SS6D_TE:
        used_cfg = SPLITS_SS6D_TE[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, SS6D_Dataset(used_cfg))
    ...
    MetadataCatalog.get(name).set(
        ...
        evaluator_type="bop",
        **get_SS6D_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_SS6D_TE.keys())
    ...
    for d in dicts:
        ...
        for _i in range(len(annos)):
            ...
            grid_show(
                ...
                col=3,
                show=False,
                save_path=osp.join(dirname, f"ss6d_pbr_{_i}.jpg"),
            )
