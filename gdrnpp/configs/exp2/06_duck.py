...
OUTPUT_DIR = "output/gdrn/SS6D/ICLR_inference/exp2/duck"
INPUT = dict(
    DZI_PAD_SCALE=1.5,
    TRUNCATE_FG=False,
    CHANGE_BG_PROB=0.0,
...
DEPTHs = ["0"]
TEST = tuple(f"ss6d_duck_{lv.lower()}{rgb.lower()}_d{depth}_te" for lv in LEVELS for rgb in TE_RGBs for depth in DEPTHs)

DATASETS = dict(
    TRAIN=("ss6d_duck_train_pbr",),
    TRAIN2 = tuple(f"ss6d_duck_lv3{rgb.lower()}_tr" for lv in LEVELS for rgb in TR_RGBs),
    TRAIN2_RATIO=0.03125,

    #TRAIN2 = tuple(f"ss6d_duck_{lv}{rgb.lower()}_tr" for lv in LEVELS for rgb in TR_RGBs),
