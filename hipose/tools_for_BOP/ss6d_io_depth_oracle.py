...
# 수정
def get_dataset(bop_dir,dataset,train=True,incl_param=False ,eval_model=False, data_folder='None', data_per_obj=False, train_obj_visible_theshold=0.1, depth_shuffle=False, general=True, brightness='B50', sensor=None, depth_preset_choice=None):
    """
                    ...
                    for img_id in sorted(scene_params.keys()): # 000000.png
                        ...
                        for rgb_dir in os.listdir(rgb_path): # AE
                            ...
                            if depth_shuffle:
                                ...
                                if depth_preset_choice is None:
                                    print("depth_presets[0]: ", depth_presets[0])
                                    depth_fn = os.path.join(depth_path, depth_presets[0], "{:06d}.png".format(im_id))
                                else:
                                    print("depth_preset_choice: ", depth_preset_choice)
                                    depth_fn = os.path.join(depth_path, depth_preset_choice, "{:06d}.png".format(im_id))
