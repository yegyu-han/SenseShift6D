...
BOP_LIST = ['lmo','tless','ycbv','hb','tudl','icbin','itodd','ss6d']
...
def get_bop_reader(video_dir, zfar=np.inf):
  ...
  if 'ss6d' in video_dir:
    return SS6DReader(video_dir, zfar=zfar) #수정
  else:
    raise RuntimeError
...
def get_bop_video_dirs(dataset):
  ...
  elif dataset=='ss6d': # 수정
    video_dirs = sorted(glob.glob(f'{BOP_DIR}/ss6d/test/*'))
  else:
    raise RuntimeError
  return video_dirs
...
class SS6DReader:
    def __init__(self, root_dir, brightness='B50', general=True, sensor=None, depth_shuffle=False, obj_id=0, depth_preset_choice=0, use_detection_mask=False, det_json_path=None):
        self.rgb_files = []
        self.depth_files = []
        self.mask_files = []
        self.poses = []
        ...
        self.obj_id = obj_id  # 0 ~ 4
        preset_counts = {}

        self.use_detection_mask = use_detection_mask
        self.detection_data = None

        if self.use_detection_mask:
          if det_json_path is None:
            raise ValueError("[Error] Invalid detection json path.")
          if not os.path.exists(det_json_path):
            raise FileNotFoundError(f"Detection JSON 경로가 존재하지 않음: {det_json_path}")
          with open(det_json_path, 'r') as f:
            ...
            if isinstance(raw_data, list):
                ...
                for item in raw_data:
                    if not isinstance(item, dict) or 'scene_id' not in item or 'image_id' not in item:
                        continue 
                    
                    key = f"{item['scene_id']}/{item['image_id']}"
                    if key not in self.detection_data:
                        self.detection_data[key] = []
                    self.detection_data[key].append(item)
            elif isinstance(raw_data, dict):
                self.detection_data = raw_data
            else:
                raise TypeError(f"지원하지 않는 JSON 형식입니다: 리스트 또는 딕셔너리가 필요하지만, {type(raw_data)}가 감지되었습니다.")

        for brightness_dir in sorted(os.listdir(root_dir)):
            if brightness != 'ALL' and brightness_dir != brightness:
                continue
            
            brightness_path = os.path.join(root_dir, brightness_dir)
            if not os.path.isdir(brightness_path):
                continue

            for scene in sorted(os.listdir(brightness_path)):
                scene_path = os.path.join(brightness_path, scene)
                cam_path = os.path.join(scene_path, 'scene_camera.json')
                gt_path = os.path.join(scene_path, 'scene_gt.json')
                print("gt_path: ", gt_path)

                if not os.path.exists(cam_path):
                    print(f"[누락] scene_camera.json 없음: {scene}")
                if not os.path.exists(gt_path):
                    print(f"[누락] scene_gt.json 없음: {scene}")

                if not os.path.exists(cam_path) or not os.path.exists(gt_path):
                    continue
                ...
                if "0_G32" in depth_presets:
                    depth_presets.remove("0_G32")

                for img_id in sorted(scene_camera.keys()):
                    im_id = int(img_id)

                    for rgb_dir in os.listdir(rgb_path):
                        if general:
                            if rgb_dir != 'AE':
                                continue
                            if brightness != 'ALL' and brightness_dir != brightness:
                            ...
                            if sensor is not None and rgb_dir != sensor:
                                continue
                        ...
                        rgb_file = os.path.join(rgb_path, rgb_dir, f'{im_id:06d}.png')

                        if depth_shuffle:
                            if not preset_counts:
                                preset_counts = {p: 0 for p in depth_presets}
                            min_count = min(preset_counts.values())
                            least_used = [p for p in depth_presets if preset_counts[p] == min_count]
                            chosen_preset = np.random.choice(least_used)
                            preset_counts[chosen_preset] += 1
                        else:
                            chosen_preset = depth_preset_choice

                        depth_file = os.path.join(depth_path, chosen_preset, f'{im_id:06d}.png')
                        mask_file = os.path.join(mask_path, f'{im_id:06d}_000000.png')

                        if not os.path.exists(mask_file):
                          print(f"[경고] mask 파일 없음: {mask_file}")

                        if not (os.path.exists(rgb_file) and os.path.exists(depth_file) and os.path.exists(mask_file)):
                            continue

                        gt = scene_gt[img_id][0]  
                        if gt['obj_id'] != self.obj_id + 1:  
                          continue 
                        ...
                        K = np.array(scene_camera[img_id]['cam_K']).reshape(3, 3)
                        self.Ks.append(K)
 
                        R = np.array(gt['cam_R_m2c']).reshape(3, 3)
                        t = np.array(gt['cam_t_m2c']).reshape(3, 1)
                        t = np.array(gt['cam_t_m2c']).reshape(3, 1) / 1000.0
                        pose = np.eye(4)
                        pose[:3, :3] = R
                        pose[:3, 3:] = t
        ...
        print(f'[SS6DReader] 총 {len(self.rgb_files)}개 프레임 로드 완료.')
    ...
    def get_depth(self, i):
        import imageio
        depth = imageio.imread(self.depth_files[i]).astype(np.float32)
        depth /= 1000.0
        return depth
    ...
    def get_mask(self, i):
        ...
        if self.use_detection_mask:
            scene_id = 0 
            frame_id = int(self.ids[i])
            key = f"{scene_id}/{frame_id}"
            
            print(f"[DEBUG] i={i}, scene_id={scene_id}, frame_id={frame_id}, JSON key: {key}")
            
            detections = self.detection_data.get(key, [])
            if not detections:
                print(f"[경고] JSON 키 '{key}'에 해당하는 탐지 결과가 없음")
                return np.zeros((720, 1280), dtype=bool)

            for det in detections:
                print(f"[CHECK] - det category_id: {det['category_id']} (target: {self.obj_id + 1}), score: {det['score']}, segmentation: {det['segmentation']}")
                
                if det["category_id"] == self.obj_id + 1:
                    segm = det["segmentation"]
            ...
            print(f"[경고] 키 '{key}'의 탐지 결과에 타겟 ID ({self.obj_id + 1})가 없습니다. 빈 마스크를 반환합니다.")
            return np.zeros((720, 1280), dtype=bool)
        else:
            print("use GT mask")
            return imageio.imread(self.mask_files[i])
    ...
    def get_GT(self, i):
      gt_R = self.poses[i][:3,:3]
      gt_T = self.poses[i][:3,3]
      return gt_R, gt_T
